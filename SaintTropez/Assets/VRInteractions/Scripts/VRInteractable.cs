...
public class VRInteractable : ExposableMonobehaviour {
	...
	{
		foreach (SteamVR_Behaviour_Pose controller in VRGripper.GetControllers())
		{
			Collider[] controllerColliders = controller.gameObject.GetComponentsInChildren<Collider>();
			IgnoreColliders(controllerColliders);
		}
	}
	...
	void RemoveIgnoreAllControllerColliders()
	{
		...
		{
			Collider[] controllerColliders = controller.gameObject.GetComponentsInChildren<Collider>();
			RemoveIgnoreColliders(controllerColliders);
		}
	}

	// Adjusted IgnoreColliders and RemoveIgnoreColliders methods to accept an array of Colliders directly
	void IgnoreColliders(Collider[] colliders)
	{
		IgnoreColliders3D(colliders, mColliders3D.ToArray());
	}

	void RemoveIgnoreColliders(Collider[] colliders)
	{
		IgnoreColliders3D(colliders, mColliders3D.ToArray(), false);
	}
	/// <summary>
